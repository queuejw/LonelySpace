...
from base.game.classes.game import clamp

# Класс события на планете
class PlanetEvent:
    def __init__(self, m_event_name: str, m_event_description: str, m_event_command: str, m_prob: float):
        ...
        self.event_command: str = m_event_command  # Команда для генерации события.
    ...
    def run_event(self) -> bool:
        if random.random() < self.event_prob:
            ...
            return False
        command = self.event_command.split()
                    ...
                    case "remove":
                        ...
                        try:
                            ...
                            components.GAME.player.resources = clamp(
                                components.GAME.player.resources - random.randint(base_size + var, base_size - var), 0,
                                99999)
                        except ValueError:
                            # Вероятно, ошибка при выполнении str -> int или str -> float
                            if components.SETTINGS.get_debug_mode():
                    ...
                    case "add":
                        ...
                        try:
                            ...
                            components.GAME.player.strength = clamp(
                                components.GAME.player.strength + random.randint(base_size + var, base_size - var), 0,
                                100)
                        except ValueError:
                            # Вероятно, ошибка при выполнении str -> int или str -> float
                            if components.SETTINGS.get_debug_mode():
